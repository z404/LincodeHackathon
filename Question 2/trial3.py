...
for c in contours:
    ...
    if (w > 20 and h > 20): #and w > 3*h:
        ...
        string_in_img = pytesseract.image_to_string(r'cropped/'+str(idx)+'.png')
        print(string_in_img)
        if 'nutri' in string_in_img.lower() and (('sugar' in string_in_img.lower()) or ('carb' in string_in_img.lower())):
            cv2.imwrite(detected+str(idx) + '.png', new_img)
    print(w,h, idx)
